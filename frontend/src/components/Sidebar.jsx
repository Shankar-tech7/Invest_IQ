...
import { ShieldAlert, Activity, FileText, Video, Network, Clock, Settings, LogOut, Cpu, Layers, User } from "lucide-react";
...
export default function Sidebar() {
  ...
  const handleLogout = () => {
    fetch("http://127.0.0.1:5000/api/auth/logout", { method: "POST" })
      .then(() => {
  ...
  return (
    <aside className="w-64 flex flex-col bg-[#000000] border-r border-[#171717] shadow-xl relative z-10">
      <div className="p-6 flex items-center gap-3 border-b border-[#171717]">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-wider text-slate-100 uppercase">InvestIQ</h1>
          <p className="text-[10px] text-[#ff003c] uppercase tracking-widest">Forensic AI</p>
        </div>
      </div>
      
      <div className="px-6 py-3 bg-[#ff003c]/5 border-y border-[#ff003c]/10 mb-2">
        <p className="text-[9px] text-[#ff003c] font-mono tracking-tighter opacity-70">SYSTEM_ID: INVESTIQ_PROT_V4</p>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn("w-5 h-5", isActive ? "text-[#ff003c]" : "group-hover:text-slate-200")} />
              {item.name}
            </Link>
          );
        })}
